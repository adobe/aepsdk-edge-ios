...
# SOUCE_CODE_VERSION_IN_AEPEdge=$(cat ./AEPSignal/Sources/SignalConstants.swift | egrep '\s*EXTENSION_VERSION\s*=\s*\"(.*)\"' | ruby -e "puts gets.scan(/\"(.*)\"/)[0] " | tr -d '"')
...
if [[ "$1" == "$PODSPEC_VERSION_IN_AEPEdge" ]]; then
    echo "${GREEN}Pass!${NC}"
else
