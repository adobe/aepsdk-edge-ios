...
struct KonductorConfig {
    ...
    enum CodingKeys: String, CodingKey {
        case imsOrgId = "imsOrgId"
        case streaming = "streaming"
    }
}

extension KonductorConfig : Encodable {
    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        if let unwrapped = imsOrgId { try container.encode(unwrapped, forKey: .imsOrgId)}
        if let unwrapped = streaming { try container.encode(unwrapped, forKey: .streaming)}
    }
}

extension KonductorConfig : Decodable {
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        imsOrgId = try? container.decode(String.self, forKey: .imsOrgId)
        streaming = try? container.decode(Streaming.self, forKey: .streaming)
    }
}
